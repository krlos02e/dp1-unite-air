...
type Page = 'inicio' | 'carga' | 'dashboard' | 'simulacion' | 'colapso'

function App() {
  const [page, setPage] = useState<Page>('inicio')
  const [sessionId, setSessionId] = useState<string>('')
  const [colapsoState] = useState<SimulationState | null>(null)
        ...
        {checking ? (
          ...
          <>
            <nav className="bg-gray-900 border-b border-gray-800 px-4 sm:px-6 py-3 flex items-center gap-4 sm:gap-6 overflow-x-auto">
              ...
              <button onClick={nav('simulacion')} className="text-sm sm:text-base hover:text-sky-300 shrink-0">Simulación</button>
              <button onClick={nav('dashboard')} className="text-sm sm:text-base hover:text-sky-300 shrink-0">Dashboard</button>
            </nav>
            ...
            <main className="p-4 sm:p-6 flex-1">
              {page === 'inicio' && (
                <div className="text-center mt-10 sm:mt-20">
                  ...
                  <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center items-stretch sm:items-center px-4">
                    ...
                    <button onClick={nav('simulacion')} className="bg-emerald-600 hover:bg-emerald-700 px-6 py-3 rounded-lg font-semibold">Simulación</button>
              ...
              {!sessionId && page === 'dashboard' && (
                <div className="text-center mt-20">
                  ...
                  <button onClick={nav('simulacion')} className="bg-sky-600 px-4 py-2 rounded">Ir a Simulación</button>
                </div>
              )}
